...
class ToCLSTensor(BaseTransform):
    def transform(self, results: Dict) -> Dict:
        ...
        # results['pcd'] = torch.tensor(results['pcd']).to(torch.float32).reshape(3,-1)  # PyTorch requires (C, N) format
        ...
        # results['multi_view_imgs'] = [torch.tensor(image).to(torch.float32) for image in results['multi_view_imgs']]
        results['multi_view_imgs'] = torch.stack([torch.tensor(image) for image in results['multi_view_imgs']]).to(torch.float32)
        results['render_imgs'] = torch.stack([torch.tensor(image) for image in results['render_imgs']]).to(torch.float32).unsqueeze(1)
        return results
