...
class N2PAttention(nn.Module):
    def __init__(self):
        ...
        # self.bn1 = nn.LayerNorm([128, 2048])
        # self.bn2 = nn.LayerNorm([128, 2048])
...
class DownSample_new(nn.Module):
    def __init__(self, npts_ds):
    ...
    def forward(self, x):
        ...
        attention = self.softmax(energy / scale_factor)  # (B, N, N) -> (B, N, N)
        mask_list = [] # (B, N)
        indices_list = [] # (B, npts_ds)
        for i in range(attention.shape[0]):
            cluster_ids_x, cluster_centers, distances = kmeans.kmeans(
                X=attention[i], num_clusters=2, distance='euclidean', device=attention.device, iter_limit=1000
            )
            min_distances, _ = distances.min(dim=1)
            sorted_indices = torch.argsort(min_distances)
            top_indices = sorted_indices[self.npts_ds:]
            indices_list.append(top_indices)
            mask = torch.zeros(distances.shape[0], dtype=torch.bool) # (N)
            mask[top_indices] = True
            mask_list.append(mask)

        mask_tensor = torch.stack(mask_list) # (B, N)
        self.idx = torch.stack(indices_list) # (B, npts_ds)
        filtered_attention = torch.stack([attention[i, mask_tensor[i], :] for i in range(attention.size(0))]).contiguous() # (B, N, N) -> (B, M, N)
        v = filtered_attention @ rearrange(v, 'B C N -> B N C').contiguous()  # (B, M, N) @ (B, N, C) -> (B, M, C)
        out = rearrange(v, 'B M C -> B C M').contiguous()  # (B, M, C) -> (B, C, M)
        return out
